...
| `SearchField` | Compact search input on the FilterChip geometry — same 36px height, tag radius, 13px type, card/border treatment, plus the kit's animated focus ring (the WebKit UA focus outline is suppressed on the input), a leading search icon, and a clear button. The counterpart to a chip row: revealed by a chip or shown inline, it reads as one family with the chips beside it. The consumer owns the query state and what searching does. |
