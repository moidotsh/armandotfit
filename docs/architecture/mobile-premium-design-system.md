...
| `MobileTabBar` | Bottom chrome for apps whose primary verb deserves a permanent home in the thumb arc: four flanking tabs + a raised center action. Bar = card surface with a 1px hairline top edge + bottom safe-area inset (flat — no blur, no floating dock). Active tab: ink label + a 2px signal notch centered on the top edge; inactive: muted; labels ride `theme.fonts.mono` tracked caps (instrument markings). The center action is a 56px signal circle raised past the bar top (`alignSelf: 'stretch'` on its slot — a content-height slot sinks the button into the bar, measured), ink glyph, road-sign contrast; `centerAction.active` runs the one ambient animation in the system — a 1.6s opacity/scale breath on a ring — collapsed to static under reduced motion. The bar owns no routing: consumers pass onPress callbacks. |
...
| `MobileActionFooter` | Sticky bottom action area on an opaque page-tone plate with a top hairline (both modes — scrolled content terminates at the hairline, never collides with the actions). Holds the primary + optional secondary (secondary text reads quiet ink unless accented). 76–92px total with safe-area bottom. |
...
| `MobileSettingsRow` | Shared layout for every settings row: 36×36 accent-tinted iconBox + `flex: 1` text column + optional right element (values carry `tabular-nums` — row values often hold figures). The `flex: 1` (not `flexShrink: 1`) is load-bearing — it locks the title's x-coordinate at `iconBox.width + 12` regardless of right-element width, description length, or Pressable-vs-View wrapper. Destructive rows render with red title + red-tinted iconBox. |
...
| `SegmentedControl` | Square-cut segmented control (the active segment paints the `control` shape token — shapes govern the look, not literals). **Selection is ink inversion, never the accent**: the active segment fills `colors.text` with a page-colored label — the brand slot belongs to a screen's one verb. Labels ride `theme.fonts.mono` when a mono face is declared (selection labels are markings). Two explicit a11y variants. `variant="selection"` — radiogroup/radio for mutually-exclusive value pickers (period, scope, density). `variant="tabs"` — tablist/tab for content-region switching. The two variants share visual treatment but carry distinct a11y contracts — pick by content semantics, not by visual preference. The `tabs` variant does NOT wire `aria-controls` via a shell-managed id; the consumer owns matching panel composition and platform-specific panel association end-to-end (the shell ships the tablist + tab semantics only). `chromeless` drops the track fill for inline affordances inside a hero surface. No slide animation in v1 — the active state changes instantly. |
| `FilterChip` | Interactive marking chip (the `tag` shape token — a chip is a marking box) — one chip with label, optional icon, selected state, tap handler. Accessibility state MUST match the chosen role: `accessibilityRole="button"` (default) → `accessibilityState.selected`; `"radio"` or `"checkbox"` → `accessibilityState.checked`. The component maps the role to the correct state key; the consumer picks the role by the semantic use case (single-select cluster → radio; multi-select cluster → checkbox; standalone toggle → button). Never set both `selected` and `checked`. |
...
| `SearchField` | Compact search input on the FilterChip geometry — same 36px height, tag radius, input type at the ramp's row rank, card/border treatment, plus the kit's animated focus ring (the WebKit UA focus outline is suppressed on the input), a leading search icon, and a clear button. Focus is an ink moment (revision 2027-03, the states amendment): the accent defaults to the resolved text ink — never the brand slot, whose red reads as error — the ring snaps (duration 0, no halo glow), and it rides the host's shape token; pass `accentColor` to override. The counterpart to a chip row: revealed by a chip or shown inline, it reads as one family with the chips beside it. The consumer owns the query state and what searching does. |
