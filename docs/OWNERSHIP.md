...
| The active design thesis (THE INTERVAL: the live figure owns the counter — the two-state logger, ink as state, the ruled row, the harmonic ramp, the still system + the re-weight, one red ink strictly rationed ≤3 marks/screen; THE SIGHT AMENDMENT is its current revision) | `docs/architecture/interval-thesis.md` | Supersedes `scoreboard-thesis.md`, `gauge-thesis.md`, `board-thesis.md`, `quiet-page-thesis.md`, `signal-thesis.md` (all retired, kept as records). |
| The design-law optical probes (the amendment's DOM-truth assertions: figure truncation, counter face, board ink, red ration, sheet cut, footer ground, re-weight both directions) | `.shots/fit-walk.ts` (scratch, uncommitted) + `scripts/verify-design.ts` (the gated source-law subset) | The thesis's §10 lists what MUST be asserted; the walk script owns how. |
