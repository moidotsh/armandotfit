// constants/theme.ts
// armandotfit theme — "THE BROADSHEET" (see
// docs/architecture/broadsheet-thesis.md).
//
// The daily training paper on the shell's `ink` dialect (flat
// atmosphere, chit toasts, curtain transitions — see DIALECT below):
//
//   • THE PAGE — one continuous warm sheet ruled by hairlines:
//     PAPER in the light (warm cream + warm ink), EVENING EDITION at
//     night (warm black + cream text). Never cool gray, never
//     blue-steel. Square-cut print surfaces (2px), no shadows.
...
// living pulse. Every text-bearing slot is measured WCAG AA on the
// darkest surface it rides (the full matrix lives in the thesis §3.3
// and is regenerated by the contrast probe before any palette
// commit): light record #C24100, recordText #9A3300, paper
// textOnRecord on the fill; dark record #FF7E45, recordText #FF9E6E,
// ink-on-record; wire text #EDE9DE on #0E0C08.
//
...
export interface TypeFaces {
   ...
   * THE BROADSHEET leaves this undefined: its display face (Rokkitt)
...
const FONTS = {
  // THE BROADSHEET pair (docs/architecture/broadsheet-thesis.md §2.1).
  // Rokkitt — the slab-serif statement voice — speaks headlines
  // (screen statements, station names, hero figures, button labels).
  // Azeret Mono keeps the agate (every changing or aligned figure:
  // timers, set rows, the armed call — mono is tabular by
...
export const theme = {
  ...
  // ── Named type styles ─────────────────────────────────────────────────
  // THE BROADSHEET scale (docs/architecture/broadsheet-thesis.md §2.2):
  // three interleaved ramps — statements in Rokkitt (28 · 34 · 42 · 72),
  // agate figures in Azeret Mono (14 · 56, always tabular), words
  // (11 · 13 · 15 · 16 · 17). The headline law: the loudest size on a
  // screen is unique and ≥ 1.4× the next distinct size. Line-heights
  // snap to 2. Consumers import the named style and spread it; they do
  // NOT pick ad-hoc fontSize/fontWeight values. Retune the values here,
  // but keep the named-style discipline.
  typography: {
    mobileTitle: {
      fontSize: 28,
      fontWeight: '700',
    ...
    // station name, one register under the headline scale.
    ...
    // one place the mono face is allowed above 30px.
    mobileCounter: {
      fontSize: 56,
      fontWeight: '700',
      lineHeight: 60,
