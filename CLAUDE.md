...
2. **PWA-only (no native build artifacts); not mobile-only.** `app.config.ts` is web-static-export only (`output: "static"`). The runtime manifest-injection block in `app/_layout.tsx` is load-bearing — Expo Web's static export strips `<link rel="manifest">` from `dist/index.html`, and runtime injection restores it. Don't remove that block without reading `docs/architecture/pwa-installability.md` first. Desktop-shaped layouts (multi-column, sidebar nav) are a consumer extension that adds a sibling `DesktopPremium` kit — see `docs/contributing.md` → "Adding desktop support". "PWA-only" forbids iOS/Android native build artifacts; it does NOT forbid desktop browser layouts.
