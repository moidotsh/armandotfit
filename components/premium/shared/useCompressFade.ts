...
//
// M3 — THE COMPRESS (the shared motion primitive behind it): a fade
// value that runs 0 while a page's hero statement is on screen and 1
// once it has scrolled one runway past. Scaffolds mount a compact
// restatement bar whose opacity reads this value — the big words hand
// off to the carried words.
//
// Contracts (the repo's motion law): transform/opacity only, no
// layout properties; DOM-safe (the Animated.Value exists everywhere,
// the listener only updates where scroll fires); under reduced motion
// — or off-web — the consumer renders its restatement statically at
// full opacity, so the design is complete without the fade.
